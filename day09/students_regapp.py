...
def getData(event):
    ...
    ##  엔트리 위젯에 각각 채워넣기
    ent1.config(state='normal')     # 데이터 들어갈 수 있도록 해주고
...
def modData():
    ...
    if std_id == '':
        messagebox.showwarning('UPDATE', '수장할 데이터를 선택요망!')
        return
...
def delData():
    ...
    if std_id == '':
        messagebox.showwarning('DELETE', '삭제할 데이터를 선택요망!')
        return
...
ent1.config(state='readonly', foreground='blue', disabledbackground='blue')   # 값을 입력 못하게 방지
...
# 6. 트리뷰 항목 더블클리하면 이벤트 발생(getDatata()함수 호출)
