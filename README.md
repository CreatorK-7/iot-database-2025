...
- MySQL 설치(Docker)
    1. 파워쉘을 오픈, 도커 확인
        ...
        ```
        powershell에서 도커 실행 확인 -> docker ps  
...
- 데이터베이스 프로그래밍
    ...
    - 사용자 정의 함수 : [SQL](./day05/db_03_사용자정의함수.sql)
        - 리턴 키워드 사용
...
- 데이터모델링
    ...
    - 모델링 순서
        ...
        3. 물리적 모델링 - DMBS 종류에 맞게 데이터타입 지정, 물리적 구조 정의. 응답시간 최소화/트랜잭션 검토/저장공간 배치
    - 현재 ER다이어그램은 IE(Information Engineering) 방식으로 설계
    - ERWin 설계 실습: [ERWin](./day05/madang.erwin)
...
- 정규화 : [SQL](./day06/db03_정규화_이상현상.sql)
    ...
    - 함수 종속성 - 하나의 속성(A)이 다른 속성(B)을 결정지으면 A는 B의 결정자. A → B. A가 기본키가 됨
        ...
        - BCNF에 해당하지 않는 경우 - A → C, B → C일 경우 C → B 경우가 발생 
    - 정규화 - 이상현상이 발생하지 않도록 릴레이션(개체, 테이블)을 분해하는 과정
        - 제1정규화 - 속성이 원자값을 가지도록 만드는 정규화(한 컬럼에 여러값이 들어갈 수 없음)
        - 제2정규화 - 모든 릴레이션의 속성이 완전함수 종속을 만족하는 정규화(테이블 분해)
        - 제3정규화 - 이행적종속이 발생하지 않도록 정규화(테이블 분해)
...
- 트랜잭션 : [SQL](./day06/db05_트랜잭션2.sql)
    ...
    - 특징
        ...
        - C(일관성) - Consistency. 트랜잭션 전후 데이터가 일관되게 저장되어 있어야 함.
        - I(고립성) - Isolation. 트랜잭션이 발생할 동안 다른 트랜잭션이 값을 수정하지 못하게 막음.
        - D(지속성) - Durability. 트랜잭션 후 저장된 데이터는 무한히 그 값이 유지되어야 함.
    - 동시성 제어 : [SQL](./day06/db09_동시성제어1.sql) / [SQL2](./day06/demo_동시성제어2.sql)
        - 락
...
- Workbench Tip
    ...
    - 성능테스트 시 1000개 제한 풀어줘야 함
    - 메뉴 `Edit> Preferences > SQL Editor > SQL EXcution 에서 Limit Rows Count`를 조절

    <img src="./image/Workbench_tip.png" width = "600">
...
- tkinter DB연동 GUI앱 개발 : [Python](./day09/students_regapp.py)
    1. MySQL madan 데이터베이스 사용하는 madang 사용자 생성, 권한 
    2. madang DB에 students 테이블 생성
        ...
        -- 더미데이터 추가 
        insert int students(std_name, std_mobile, std_regyear)
    ...
    4. 데이터 베이스 CRUD 함수 구현
        - select 기능 구현

        <img src="./image/db0008.png" width = "700>
- 데이터베이스 연습
    - SQL, 모델링 연습
- 코딩테스트
