-- INSERT
-- 3-44 Book테이블에 '스포츠 의학' 도서를 추가하세요. 한솔의학서적에서 출간했고, 90,000원 입니다.
...
-- 한 테이블에 있는 많은 데이터를 다른 테이블로 복사하ㅏ는 데 가장 효과적인 방법
INSERT INTO Imported_Book (bookid, bookname, publisher, price)
SELECT bookid, bookname, publisher, price
  FROM Imported_Book;
  
-- 추가. 테이블의 숫자형 타입으로 된 PK값이 자동을 증가하도록 만들고 사용하려면...
CREATE TABLE NewBook(
	bookid 		integer 	primary key auto_increment,	-- auto_increment는 숫자 자동증가 조건check
...
-- 3-50 NewBookd의 모든 데이터를 삭제하시오.
-- 하지 마세요ㅠ WHERE절로 필요한 데이터만 삭제하세요
-- DELETE FROM NewBook;
