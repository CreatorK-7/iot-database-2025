...
-- MySQL 문자열은 '홑따옴표 사용 가능, " 쌍따옴표 사용불가!
...
-- 3-17 고객이 주문한 도서의 총판매약, 평균값, 최저, 최고가를 구하시오.
...
-- 그룹화 GROUP BY 키워드 사용
-- 3-19 고객별로 주문한 도서의 총 수량과 총판매액을 구하시오.
...
-- 추가 : 3-19의 내용에서 고객별총액을 내림차순을 출력하시오.
SELECT custid
	 , COUNT(*) AS 구매도서수량	-- 함수 매개변수로 *, custid
	 , SUM(saleprice) AS 고객별총액
  FROM Orders
...
-- 추가 : 고객별 구매수량과 구매총액을 출력하고 전체를 합산한 통계를 표시하세요.
SELECT custid
	 , COUNT(*) AS 구매도서수량	-- 함수 매개변수로 *, custid
	 , SUM(saleprice) AS 고객별총액
  FROM Orders
...
-- 3-21 : 고객과 고객의 주문에 환한 데이터를 나타내시오.
...
SELECT c.custid
 ...
 -- 필요하면 테이블 조인하면 됨SELECT c.custid
 SELECT c.custid
	 , c.name
     , c.address
     , o.orderid
...
-- 3-23 고객의 이름과 주문한 도서의 판매가격을 검색하시오.
...
-- EXISTS : 상관서브쿼리에서 사용하는 키워드. 상관서브쿼리가 아니더라도 사용가능
