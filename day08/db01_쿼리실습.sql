...
-- 서브쿼리(부속질의) 계속
/* 문제1 - 사원의 급여 정보 중 업무별(job) 최소 급여를 받는 사원의 성과 이름을 Name으로 별칭, 업무, 그여, 입사일을 출력(21행)
*/
...
/* 샘플문제1 - 프로젝트 성공으로 급여인상이 결정됨.
	사원은 현재 107명, 19개 업무에 소속되어 근무 중. 회사 업무 Distict job_id 5개 업무에서 일하는 사원.
...
/* 문제3 - 월별로 입사만 사원수가 아래와 같이 행별로 출력되도록 하시오.(12행)
*/
-- 형변환 함수 cast(), convert()
select cast('-09' as unsigned);	-- signed(음수 포함 숫자형)
select convert('09', signed);	-- unsigned(양수만 표현하는 숫자형)
...
SELECT department_id, job_id
  ...
  from employees
 group by department_id, job_id
  with rollup;	-- group by의 컬럼이 하념ㄴ 총계는 하나, 컬럼이 두개면 첫번째 컬럼별로 소계, 두 컬럼의 합산이 총계로
...
SELECT case grouping(department_id) when 1 then '(ALL-DEPs)' else ifnull(department_id, '부서없음') end as 'Dept#'
	 ...
	 , concat('$', format(sum(salary), 0)) as 'Salary SUM' 
     , count(employee_id) as 'COUNT EMPs'
	 -- , grouping(department_id)	-- GOUP BY와 with rollup을 사용할 때 그룹핑이 어떻게 되는지 확인하는 함수
     -- , grouping(job_id)
...
/* 문제1 - 부서별 급여를 기준으로 내림차순 정렬하시오. 이 때 세가지 함수를 이용하여 순위를 출력하시오.(107행)
*/
select employee_id
	 , last_name
     , salary
     , department_id
     , rank() over (partition by department_id order by salary desc) as 'Rank'	-- 1, 1, 3 순위를 매기기 rank
     , dense_rank() over (partition by department_id order by salary desc) as 'Dense_Rank'	-- 1, 1, 2 순위를 매기기 dense_rank
